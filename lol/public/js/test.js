...
(function () {
    ...
    var ActionJs = {
        ...
        checkLoadOver: function () {
            if (window['action'] && window['match']) {
                ...
                this.SCTH = match;
                var temp,remain;
                //遍历寻找"正在进行"和"长期活动",特殊处理不定期活动
                this.ZZJX = [];
                this.CQHD = [];
                for (var i = 0, j = this.All.length; i < j; ++i) {
                    temp = this.All[i];
                    remain = window.activityDataHandle.culRemainTime(temp);

                    if (+temp.iDate === -2 && +temp.iStatus === 998) {
                        //标记不定期活动,没有结束时间的,标记为不定期活动
                        remain === '' && (temp.isUnknown = true);
                        //没有结束时间的和结束时间大于0的(有确定结束时间的转为普通活动处理)放入正在进行
                        (remain === '' || remain > 0) && this.ZZJX.push(temp);
     ...
     * **/
    var EventJs = {
        matchId: 113,//官网每次只显示一种赛事，这个赛事可能分为很多子赛事，如常规赛，季后赛等等，由gameType决定。
        ...
        loadConfigSuccess: function () {
            //如果是德玛西亚杯西安站,换tab名字
            if (this.matchId === 113) {
                this.configData[0].GameTypeName = '双败淘汰赛';
            }
